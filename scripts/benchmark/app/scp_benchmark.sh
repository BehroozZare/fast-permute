...
INPUT_ROOT="${APPS_ROOT}/SCP"
OUTPUT_ROOT="/home/behrooz/Desktop/Last_Project/gpu_ordering/output/Apps"
OUTPUT_CSV_BASE="${OUTPUT_ROOT}/scp"
BENCHMARK_BIN="/home/behrooz/Desktop/Last_Project/gpu_ordering/cmake-build-release/benchmark/multiple_factorization/gpu_ordering_multi_scp_benchmark"
...
mapfile -t DATASETS < <(
    for candidate in "${INPUT_ROOT}"/*; do
        ...
        if [[ -f "${candidate}/SCP_hessian.mtx" ]] && compgen -G "${candidate}/scp_rhs_*.mtx" >/dev/null; then
            printf '%s\n' "${candidate}"
        fi
    done
)
...
if [[ ${#DATASETS[@]} -eq 0 ]]; then
    echo "No SCP datasets found under ${INPUT_ROOT}"
    exit 1
fi

echo "Found ${#DATASETS[@]} SCP datasets."
...
echo "SCP benchmark complete. CSVs are in ${OUTPUT_ROOT}"
