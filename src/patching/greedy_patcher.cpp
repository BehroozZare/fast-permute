                  ...
                  int               patch_size)
{
    ...
    size_t head = 0;
    while (head < queue.size() && static_cast<int>(queue.size()) < patch_size) {
        const int v = queue[head++];
        for (int e = Gp[v];
             e < Gp[v + 1] && static_cast<int>(queue.size()) < patch_size;
             ++e) {
            const int u = Gi[e];
            if (u < 0 || u >= n) {
                continue;
...
void GreedyPatcher::compute(int               n,
                            const int*        Gp,
                            const int*        Gi,
                            std::vector<int>& node_to_patch)
{
    ...
    if (patch_size <= 0) {
        throw std::invalid_argument(
            "GreedyPatcher: patch_size must be positive");
    }
...
#pragma omp for schedule(dynamic, std::max(1, per_thread_seeds / 4))
            for (int s = 0; s < num_seeds; ++s) {
                ...
                int       expected = -1;
                const int my_patch =
                    next_patch_id.fetch_add(1, std::memory_order_relaxed);
                ...
                total_assigned.fetch_add(local_assigned,
                                         std::memory_order_relaxed);
            }
        }
...
#pragma omp parallel for schedule(static)
        ...
        for (int v = 0; v < n; ++v) {
            ...
            if (id_map[pid] == -1) {
                // Will be assigned on second pass after we know the final
                // count.
                id_map[pid] = 0;
            }
        }
...
#pragma omp parallel for schedule(static)
    ...
    // Serial cleanup pass where any vertex still tagged -1 (because it was
    // outside the parallel pass's reachable region) gets a new patch grown from
    // it. This loop is identical to the original serial BFS but bounded to the
    ...
    // when the parallel pass left holes).
    {
        ...
        int next_seed = 0;
        while (next_seed < n) {
            while (next_seed < n && node_to_patch[next_seed] != -1)
                ++next_seed;
            if (next_seed >= n)
                break;
            bfs_one_patch(n,
    ...
    if (std::any_of(node_to_patch.begin(), node_to_patch.end(), [](int patch) {
            return patch < 0;
        })) {
        throw std::runtime_error(
            "GreedyPatcher: failed to assign all vertices");
    }

    const double elapsed_ms =
        std::chrono::duration<double, std::milli>(Clock::now() - start).count();
    spdlog::info(
        "GreedyPatcher: use_parallel= {}, n={} patch_size={} patches={} "
        "time={:.3f} ms",
