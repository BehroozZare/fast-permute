...
#include "homa/ordering/cpu_ordering_with_patch.h"
...
CPUOrdering_PATCH::CPUOrdering_PATCH()
    : _Gp(nullptr), _Gi(nullptr), _G_n(0), _G_nnz(0)
{
}
...
void CPUOrdering_PATCH::local_permute_metis(int               G_n,
                                            int*              Gp,
                                            int*              Gi,
                                            std::vector<int>& local_permutation)
{
    ...
    int status = METIS_NodeND(
        &N, Gp, Gi, NULL, NULL, local_permutation.data(), tmp.data());
    if (status != METIS_OK) {
...
void CPUOrdering_PATCH::local_permute_amd(int               G_n,
                                          int*              Gp,
                                          int*              Gi,
                                          std::vector<int>& local_permutation)
{
...
void CPUOrdering_PATCH::local_permute_unity(int               G_n,
                                            int*              Gp,
                                            int*              Gi,
                                            std::vector<int>& local_permutation)
{
...
void CPUOrdering_PATCH::local_permute(int               G_n,
                                      int*              Gp,
                                      int*              Gi,
                                      std::vector<int>& local_permutation)
{
    if (this->local_permute_method == "metis") {
        ...
        const double global_avg_degree =
            this->_G_n > 0 ? static_cast<double>(this->_G_nnz) /
                                 static_cast<double>(this->_G_n) :
                             0.0;
        const int serial_amd_min_nodes =
            (this->_G_n < 3000 || global_avg_degree >= 5.5) ? 256 : 512;
        const int amd_min_nodes = use_serial_ordering_path() ?
                                      serial_amd_min_nodes :
...
void CPUOrdering_PATCH::compute_local_quotient_graph(
    std::vector<int>&
         assigned_g_node,  ///<[in] The index of the current decomposition node
    int& local_Q_n,
    std::vector<int>& local_Qp,
    std::vector<int>& local_Qi,
    std::vector<int>& local_Q_node_weights,  ///<[out] The node weights of the
                                             ///< local quotient graph
    std::vector<int>&
        q_local_to_global_map  ///<[in/out] The map from current assigned
                               ///< quotient nodes to tree nodes
)
{
    local_Qp.clear();
    local_Qi.clear();
    ...
    int cnt = 0;
    for (size_t local_q = 0; local_q < q_local_to_global_map.size();
         local_q++) {
        int global_q = q_local_to_global_map[local_q];
        local_Q_node_weights[local_q] =
            this->_quotient_graph._Q_node_weights[global_q];
        for (int nbr_ptr = this->_quotient_graph._Qp[global_q];
             nbr_ptr < this->_quotient_graph._Qp[global_q + 1];
             nbr_ptr++) {
...
void CPUOrdering_PATCH::compute_bipartition(int               Q_n,
                                            int*              Qp,
                                            int*              Qi,
                                            ...
                                            std::vector<int>& Q_partition_map)
{
    ...
    int metis_status = METIS_PartGraphKway(
        &nvtxs,
        &ncon,
        Qp,
        Qi,
...
void CPUOrdering_PATCH::two_way_Q_partition(
    ...
    std::vector<int>&
        assigned_g_nodes,    ///<[in] Assigned G nodes for current decomposition
...
)
{
    ...
    if (tree_node_idx != 0) {
        ...
        compute_bipartition(local_Q_n,
                            local_Qp.data(),
                            local_Qi.data(),
                            local_Q_node_weights,
                            two_way_q_partition_map);
        ...
        compute_bipartition(_quotient_graph._Q_n,
                            _quotient_graph._Qp.data(),
                            _quotient_graph._Qi.data(),
                            _quotient_graph._Q_node_weights,
                            two_way_q_partition_map);
        where.resize(assigned_g_nodes.size());
...
void CPUOrdering_PATCH::find_separator_superset(
    ...
    std::vector<int>&
        separator_superset  ///<[out] The superset of separator nodes
)
{

    separator_superset.clear();
    for (size_t local_node = 0; local_node < assigned_g_nodes.size();
         ++local_node) {
        ...
        for (int nbr_ptr = this->_Gp[g_node]; nbr_ptr < this->_Gp[g_node + 1];
            ...
            if (this->_decompose_marker[nbr_id] != marker_token)
                continue;
...
//  std::sort(separator_superset.begin(), separator_superset.end());
    ...
    separator_superset.erase(
        std::unique(separator_superset.begin(), separator_superset.end()),
        separator_superset.end());
...
{
#ifndef NDEBUG
    spdlog::info("METIS refinement: separator size before refinement: {}",
    ...
    int status = METIS_NodeRefine(local_nvtxs,
                                  subgraph._Gp.data(),
                                  nullptr,
                                  subgraph._Gi.data(),
                                  where.data(),
                                  ...
                                  ubfactor);
    if (status != METIS_OK) {
        spdlog::error("MetisPatcher: METIS_NodeRefine failed (status={})",
                      status);
    ...
    spdlog::info("METIS refinement: separator size after refinement: {}",
                 total);
#endif
}
    ...
    int               marker_token)
{
    ...
    for (int left_idx = 0;
        ...
        for (int nbr_ptr = this->_Gp[g_node]; nbr_ptr < this->_Gp[g_node + 1];
            ...
            if (boundary_token[nbr] != current_token)
                continue;
    ...
    auto dfs = [&](auto&& self, int u) -> bool {
        for (int v : left_adj[u]) {
            const int matched_left = pair_right[v];
            if (matched_left == -1 || (dist[matched_left] == dist[u] + 1 &&
    ...
    while (head < queue.size()) {
        const int u = queue[head++];
        for (int v : left_adj[u]) {
            if (pair_left[u] == v || visited_right[v])
                continue;
            visited_right[v]       = 1;
            const int matched_left = pair_right[v];
    ...
    std::vector<int>& where)
{
    ...
    std::vector<int> separator_g_nodes;
    find_separator_superset(
        assigned_g_nodes, where, marker_token, separator_g_nodes);
    ...
    if (use_min_vertex_cover_refinement) {
        refine_separator_min_vertex_cover(tree_node_idx,
                                          assigned_g_nodes,
                                          separator_g_nodes,
                                          where,
                                          marker_token);
    } else {
        refine_separator_metis(tree_node_idx,
                               assigned_g_nodes,
                               separator_g_nodes,
                               where,
                               marker_token);
...
{
    int offset = 0;
    for (size_t i = _decomposition_tree.get_number_of_decomposition_nodes() - 1;
         i >= 0;
         i--) {
        auto& decomposition_node =
            this->_decomposition_tree.decomposition_nodes[i];
...
#pragma omp atomic
    ...
    if (tree_node_id == -1 || tree_node_parent_id == -1) {
        if (tree_node_id != 0) {
    ...
    if (level == this->_decomposition_tree.decomposition_level) {
        ...
        cur_decomposition_node.init_node(
            -1, -1, tree_node_id, tree_node_parent_id, level, assigned_g_nodes);
    ...
    const bool use_direct_separator =
        !use_patch_separator || (direct_separator_max_level >= 0 &&
    ...
    if (!use_direct_separator) {
        ...
        this->two_way_Q_partition(tree_node_id, assigned_g_nodes, where);
        double partition_ms = elapsed_ms(partition_start, Clock::now());
#pragma omp atomic
        ...
        double separator_ms = elapsed_ms(separator_start, Clock::now());
#pragma omp atomic
        ...
        int ret = METIS_ComputeVertexSeparator(&nVertices,
                                               subgraph._Gp.data(),
                                               subgraph._Gi.data(),
                                               nullptr,
                                               nullptr,
        ...
        if (ret != METIS_OK) {
            spdlog::error(
                "MetisPatcher: METIS_ComputeVertexSeparator failed (status={})",
                ret);
...
#pragma omp atomic
    ...
    this->_decomposition_tree.assign_nodes_to_tree(separator_g_nodes,
                                                   tree_node_id);
...
#pragma omp atomic
...
#ifndef NDEBUG
    spdlog::info(
        "The left size is: {}, the right size is: {} and the separator size is "
        "{}: ",
    ...
    if (left_node_idx != -1) {
        ...
        decomposition_info_stack[left_node_idx].decomposition_node_parent_id =
            tree_node_id;
    ...
    if (right_node_idx != -1) {
        ...
        decomposition_info_stack[right_node_idx].decomposition_node_parent_id =
            tree_node_id;
        decomposition_info_stack[right_node_idx].assigned_g_nodes =
            right_assigned_g_nodes;
    }

    double bookkeeping_ms = elapsed_ms(bookkeeping_start, Clock::now());
#pragma omp atomic
...
void CPUOrdering_PATCH::decompose()
{
    ...
    if (use_serial_ordering_path()) {
        for (int l = 0; l < this->_decomposition_tree.decomposition_level + 1;
             l++) {
            ...
            assert(
                end_level_idx <=
                this->_decomposition_tree.get_number_of_decomposition_nodes());
            for (int tree_node_id = start_level_idx;
                 tree_node_id < end_level_idx;
                 ++tree_node_id) {
...
#pragma omp for schedule(static)
    ...
    // auto omp_parallel_duration =
    // std::chrono::duration_cast<std::chrono::milliseconds>(omp_parallel_end -
    // omp_parallel_start).count(); spdlog::info("OpenMP parallel region wall
    // clock time: {} ms", omp_parallel_duration);
}


void CPUOrdering_PATCH::init_patches(int               num_patches,
                                     std::vector<int>& g_node_to_patch,
                                     int               num_levels)
{
...
void CPUOrdering_PATCH::step1_compute_quotient_graph()
{
    ...
    // auto patch_time =
    // std::chrono::duration_cast<std::chrono::milliseconds>(patch_fix_end -
    // patch_fix_start).count(); spdlog::info("Patch fix time: {} ms for vector
    // of size {}", patch_time, _num_patches);
    ...
    std::vector<int>              mark(
        _num_patches,
        -1);  // mark[q] == p means q already added as neighbor of p
    ...
    for (int p = 0; p < _num_patches; ++p) {
        ...
        for (int g : nodes) {
            for (int nbr_ptr = this->_Gp[g]; nbr_ptr < this->_Gp[g + 1];
                 ++nbr_ptr) {
                int nbr_id = this->_Gi[nbr_ptr];
                int q      = this->_g_node_to_patch[nbr_id];
                if (q == p)
                    continue;  // stay inside patch -> no quotient edge
    ...
    assert(cnt == q_nnz);
    this->quotient_csr_time_ms += elapsed_ms(quotient_csr_start, Clock::now());
#ifdef DEBUG
    // Check if the quotient graph CSR matrix is symmetric
    bool is_symmetric = true;
    for (int i = 0; i < _quotient_graph._Q_n; ++i) {
        for (int idx = _quotient_graph._Qp[i]; idx < _quotient_graph._Qp[i + 1];
             ++idx) {
            int j = _quotient_graph._Qi[idx];

            // Check if i exists in row j's neighbors
            bool found = false;
            for (int jdx = _quotient_graph._Qp[j];
                 jdx < _quotient_graph._Qp[j + 1];
                 ++jdx) {
            ...
            if (!found) {
                is_symmetric = false;
                spdlog::warn(
                    "CSR matrix is NOT symmetric: edge ({}, {}) exists but "
                    "({}, {}) does not",
                    i,
                    j,
                    j,
...
{
    // This loop creates the global-to-local mapping as well as number of nodes
    // per group
    ...
    for (auto& node : this->_decomposition_tree.decomposition_nodes) {
        ...
        sub_graphs[node.node_id]._Gp.assign(node.assigned_g_nodes.size() + 1,
                                            0);
    }

    // Pass 1: count degree per local node within its group to build _Gp (row
    // pointers)
    for (int g_node = 0; g_node < this->_G_n; g_node++) {
        ...
        for (int nbr_ptr = _Gp[g_node]; nbr_ptr < _Gp[g_node + 1]; nbr_ptr++) {
            ...
            int nbr_group_id =
                this->_decomposition_tree.g_node_to_tree_node[nbr_id];
    ...
    for (size_t gid = 0; gid < sub_graphs.size(); gid++) {
        if (sub_graphs[gid]._num_nodes == 0)
            continue;
...
void CPUOrdering_PATCH::step3_compute_local_permutations()
{
    ...
    auto       compute_local_ordering    = [&](int i) {
        std::vector<int> local_permutation;
        if (sub_graphs[i]._num_nodes == 0)
            return;
        auto local_order_start = Clock::now();
        local_permute(sub_graphs[i]._num_nodes,
                      sub_graphs[i]._Gp.data(),
                      sub_graphs[i]._Gi.data(),
                      local_permutation);
        this->_decomposition_tree.decomposition_nodes[i].set_local_permutation(
            local_permutation);
...
#pragma omp atomic
    ...
    if (use_serial_local_ordering) {
        for (int i = 0;
             i < this->_decomposition_tree.decomposition_nodes.size();
             i++) {
...
void CPUOrdering_PATCH::step4_assemble_final_permutation(std::vector<int>& perm)
{
    ...
    for (auto& node : this->_decomposition_tree.decomposition_nodes) {
        if (node.assigned_g_nodes.empty())
            continue;
        for (size_t local_node = 0; local_node < node.assigned_g_nodes.size();
             local_node++) {
...
void CPUOrdering_PATCH::compute_permutation(std::vector<int>& perm)
{
    ...
    if (this->_Gp == nullptr || this->_Gi == nullptr || this->_G_n == 0 ||
        this->_G_nnz == 0) {
        spdlog::error(
    ...
    // spdlog::info("Step 1 (compute quotient graph) completed in {:.6f}
    // seconds", quotient_graph_time);
    ...
    decompose_time =
        std::chrono::duration<double>(end_time - start_time).count();
    // spdlog::info("Step 2 (decomposition tree) completed in {:.6f} seconds",
    // decompose_time);
    ...
    local_permute_time =
        std::chrono::duration<double>(end_time - start_time).count();
    // spdlog::info("Step 3 (local permutations) completed in {:.6f} seconds",
    // local_permute_time);
    ...
    assemble_time =
        std::chrono::duration<double>(end_time - start_time).count();
    // spdlog::info("Step 4 (assemble permutation) completed in {:.6f} seconds",
    // assemble_time);

    ordering_total_time_ms = elapsed_ms(total_start_time, Clock::now());
    const int quotient_edges =
        _quotient_graph._Qp.empty() ? 0 : _quotient_graph._Qp.back();
