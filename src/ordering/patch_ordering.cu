...
void PatchOrdering::applyOptions(const Options& opts)
{
    _cpu_order.applyOptions(opts);
    _gpu_order.local_permute_method = _cpu_order.local_permute_method;
    _binary_level = opts.nd_levels;
    _patch_size   = opts.patch_size;
    _use_gpu      = opts.use_gpu;
}
