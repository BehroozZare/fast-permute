                          ...
                          std::vector<int>& etree)
{
    ...
    for (int i = 0; i < static_cast<int>(etree_inverse.size()); ++i) {
        ...
        for (int local = 0; local < static_cast<int>(node.assigned_g_nodes.size()); ++local) {
                             ...
                             const Options& opts)
{
    ...
    if (num_patches == 0) num_patches = 1;
                                ...
                                const Options& opts)
{
    ...
    if (static_cast<int>(node_to_patch.size()) != n)
        throw std::invalid_argument("homa::compute_ordering: node_to_patch.size() != n");
