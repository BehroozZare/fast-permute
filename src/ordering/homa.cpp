...
#include "homa/patching/greedy_patcher.h"
...
void resolve_separator_policy(int            n,
                              const int*     Gp,
                              const Options& opts,
                              ...
                              int&           min_nodes)
{
    ...
    if (n < 50000) {
        if (avg_degree < 8.0) {
            // Sparse small matrices (FEM 2D, planar meshes). Direct METIS is
            // cheap here because there are few edges; keep the original policy.
            max_level = opts.nd_levels;
        } else if (avg_degree < 20.0) {
            // Moderately dense small matrices. Use direct METIS only for the
            // top few levels where it pays off the most (gives the high-quality
            // cut at the root), then switch to quotient-based for the rest.
            max_level = 1;
        } else {
...
void apply_etree_reorder(homa::CPUOrdering_PATCH& cpu,
                         std::vector<int>&        perm,
                         std::vector<int>&        etree)
{
    ...
    for (int hmd_id = 0; hmd_id < binary_tree_size; ++hmd_id) {
        ...
        etree[level_idx] =
            static_cast<int>(nodes[hmd_id].assigned_g_nodes.size());
    }
    ...
    for (int i = 0; i < static_cast<int>(etree_inverse.size()); ++i) {
        ...
        if (node.assigned_g_nodes.empty())
            continue;
        for (int local = 0;
             local < static_cast<int>(node.assigned_g_nodes.size());
             ++local) {
...
OrderingResult run_ordering(int                     n,
                            const int*              Gp,
                            const int*              Gi,
                            ...
                            OrderingRunTiming*      timing = nullptr)
{
    ...
    resolve_separator_policy(n,
                             Gp,
                             opts,
                             cpu.use_patch_separator,
    ...
    if (timing != nullptr) {
        timing->compute_permutation_ms =
            elapsed_ms(ordering_start, Clock::now());
    }
...
void compute_node_to_patch(int               n,
                           const int*        Gp,
                           const int*        Gi,
                           const Options&    opts,
                                ...
                                const Options& opts)
{
    if (n <= 0) {
        throw std::invalid_argument(
            "homa::compute_ordering: n must be positive");
    }
    ...
    OrderingResult    result =
        run_ordering(n, Gp, Gi, node_to_patch, opts, &timing);
    spdlog::info(
                                ...
                                const Options&                     opts)
{
    ...
    return compute_ordering(
        static_cast<int>(compressed.rows()), Gp.data(), Gi.data(), opts);
}
                                ...
                                const Options&                  opts)
{
    ...
    if (A.outer == nullptr || A.inner == nullptr) {
        throw std::invalid_argument(
            "homa::compute_ordering: sparse matrix view has null pattern "
            "pointers");
    }
...
template OrderingResult compute_ordering<float>(
    const Eigen::SparseMatrix<float>&,
    const Options&);
template OrderingResult compute_ordering<double>(
    const Eigen::SparseMatrix<double>&,
    const Options&);
template OrderingResult compute_ordering<float>(const SparseMatrixView<float>&,
                                                const Options&);
template OrderingResult compute_ordering<double>(
    const SparseMatrixView<double>&,
    const Options&);
