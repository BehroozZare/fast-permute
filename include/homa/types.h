...
struct Options
{
    ...
    //   AUTO         - pick per node/level via a size/density heuristic
    //   (default) QUOTIENT     - always the (cheap) quotient-graph based
    //   separator DIRECT_METIS - always METIS_ComputeVertexSeparator on the
    //   subgraphs
    ...
    // Lloyd patcher options — effective when HOMA_WITH_LLOYD_PATCHER is
    // enabled. lloyd_iters: BFS iterations between seed-addition rounds
    // (default matches the benchmark script default of 2).
    int lloyd_iters = 2;
    int lloyd_seed  = 42;

    enum class LloydSeedMethod
    {
        RANDOM,
        MORTON,
        FPS
...
{
    std::vector<int> perm;
    // non-empty only when options.compute_etree == true
