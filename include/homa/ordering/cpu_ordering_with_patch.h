...
class CPUOrdering_PATCH
{
   public:
    ...
    struct DecompositionNode
    {
        ...
        void init_node(int               left_node_idx,
                       int               right_node_idx,
                       int               node_id,
                       int               parent_idx,
                       int               level,
    ...
    struct DecompositionTree
    {
        ...
        std::vector<DecompositionNode>
            decomposition_nodes;  // All the nodes of the tree
        std::vector<int>
            decomposition_node_offset;  // The offset of the nodes in the tree
                                        // for permutation
                                     ...
                                     int num_patches)
        {
            ...
            spdlog::info("Number of decomposition levels: {}",
                         decomposition_level);
#endif
        ...
        inline void assign_nodes_to_tree(
            const std::vector<int>& separator_g_nodes,
            int                     tree_node_id)
        {
    ...
    std::string etree_order = "post_order";  // Accept level_order and
                                             // post_order
    std::vector<int> etree;
    void             setGraph(int* Gp, int* Gi, int G_N, int NNZ);

    // This function updates the q_node_to_tree_node map for the current
    // decomposition node The left and right q nodes are mark as tree_node_idx *
    // 2 + 1 and tree_node_idx * 2 + 2 respectively
    void two_way_Q_partition(
        int tree_node_idx,  ///<[in] The index of the current decomposition node
        std::vector<int>& assigned_g_nodes,  ///<[in] Assigned G nodes for
                                             ///< current decomposition
        std::vector<int>&
            where  ///<[out] a vector with the size of assigned_g_node
    );
    ...
    void compute_bipartition(
        ...
        int* Qi,
        std::vector<int>&
            Q_node_weights,  ///<[in] The node weights of the graph
        std::vector<int>&
            Q_partition_map  ///<[out] The partition map of the graph
    );
    ...
    void find_separator_superset(
        ...
        int marker_token,  ///<[in] token marking nodes in this decomposition
                           ///< node
        std::vector<int>&
            separator_superset  ///<[out] The superset of separator nodes
    );

    // Given the separator superset and the current bipartition, refine it down
    // to a minimum vertex separator via Hopcroft-Karp maximum matching + Konig
    // minimum vertex cover of the left/right boundary bipartite graph.
    void refine_separator_min_vertex_cover(
        int tree_node_idx,  ///<[in] The tree decomposition node
        std::vector<int>&
            assigned_g_nodes,  ///<[in] All nodes in current decomposition level
        std::vector<int>&
            separator_g_nodes,    ///<[in/out] The separator nodes to refine
    ...
    // Performs balance pass + refinement pass similar to METIS separator
    // refinement
    ...
    // Given a subset of nodes (filtered nodes are marked with -1), partition
    // the graph into three parts: left, right, and separator
    void compute_vertex_separator(
        int tree_node_idx,  ///<[in] The index of the current decomposition node
        std::vector<int>& assigned_g_nodes,  ///<[in] Assigned G nodes for
                                             ///< current decomposition
        std::vector<int>& where  ///<[out] the partition assignment where = 2 is
                                 ///< separator, 0, and 1 is left and right
    );
    ...
    void compute_local_quotient_graph(
        std::vector<int>& assigned_g_node,  ///<[in] The index of the current
                                            ///< decomposition node
        int&              local_Q_n,
        std::vector<int>& local_Qp,
        std::vector<int>& local_Qi,
        std::vector<int>& Q_node_weights,  ///<[out] The node weights of the
                                           ///< local quotient graph
        std::vector<int>& Q_node_to_global_Q_node  ///<[out] The local Q node to
                                                   ///< global Q node map
    );

    void local_permute_metis(int               G_n,
                             int*              Gp,
                             int*              Gi,
                             std::vector<int>& local_permutation);

    void local_permute_amd(int               G_n,
                           int*              Gp,
                           int*              Gi,
                           std::vector<int>& local_permutation);

    void local_permute_unity(int               G_n,
                             int*              Gp,
                             int*              Gi,
                             std::vector<int>& local_permutation);

    // Apply fill-in reducing ordering to each node in the decomposition tree
    void local_permute(int               G_n,
                       int*              Gp,
                       int*              Gi,
                       std::vector<int>& local_permutation);

    // Given the full binary tree, it numbers each decomposition node in a
    // post-order manner
    int post_order_offset_computation(int offset, int decomposition_node_id);

    // Given the full binary tree, it numbers each decomposition node in the
    // tree starting from Leaves to the root
    void level_order_offset_computation();

    // Build a CSR subgraph from a set of global node IDs
    void build_subgraph_csr(
        const std::vector<int>&
                  assigned_g_nodes,  ///< [in] Global node IDs to include
    ...
    void applyOptions(const Options& opts)
    {
        ...
        switch (opts.local_method) {
            case Options::LocalMethod::AMD:
                local_permute_method = "amd";
                break;
            case Options::LocalMethod::METIS:
                local_permute_method = "metis";
                break;
            case Options::LocalMethod::NONE:
                local_permute_method = "unity";
                break;
        }
    }

    void init_patches(int               num_patches,
