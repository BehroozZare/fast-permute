...
inline void MakeSymmetricMatrixSPD(Eigen::MatrixXd& matrix)
{
    ...
    matrix = eigensolver.eigenvectors() * eigenValues.asDiagonal() *
 ...
 *
 * The output is SPD (symmetric positive definite), suitable for Cholesky
 * factorization.
 *
 * @param V  #V by 3 matrix of vertex positions
 * @param F  #F by 3 (triangles) or #F by 4 (tetrahedra) matrix of element
 * indices
 * @param L  Output SPD sparse matrix
 */
inline void computeSPD_cot_matrix(const Eigen::MatrixXd&       V,
                                  ...
                                  Eigen::SparseMatrix<double>& L)
{
    ...
    if (V.cols() != 3) {
        throw std::runtime_error(
            "SPD_cot_matrix: V must have 3 columns (3D vertices)");
    }
        ...
        if (simplex_size == 3) {
            ...
            for (int j = 0; j < C_matrix.rows(); j++)
                for (int k = 0; k < C_matrix.cols(); k++)
                    IJV.push_back(
                        Triplet<double>(F(i, j), F(i, k), C_matrix(j, k)));
        } else {
            C_matrix.setZero();
            for (int e = 0; e < edges.rows(); e++) {
                int source = edges(e, 0);
