    ...
    if(UNIX AND NOT APPLE AND _runtime_dirs)
        ...
        if(HOMA_BUILD_PYTHON AND target_name STREQUAL "_homapy")
            ...
            if(HOMA_MKL_THREADING STREQUAL "intel")
                list(APPEND _homa_mkl_component_names libmkl_intel_thread.so.2)
            elseif(HOMA_MKL_THREADING STREQUAL "sequential")
                list(APPEND _homa_mkl_component_names libmkl_sequential.so.2)
            endif()

            set(_homa_mkl_component_files)
            foreach(_name IN LISTS _homa_mkl_component_names)
                set(_found_component OFF)
                foreach(_dir IN LISTS _runtime_dirs)
                    if(EXISTS "${_dir}/${_name}")
                        list(APPEND _homa_mkl_component_files "${_dir}/${_name}")
                        set(_found_component ON)
                        break()
                    endif()
                endforeach()
                if(NOT _found_component)
                    message(FATAL_ERROR "Required MKL runtime component '${_name}' was not found in: ${_runtime_dirs}")
                endif()
            endforeach()
...
function(_homa_mkl_apply_threading_runtime target_name)
    if(HOMA_MKL_THREADING STREQUAL "intel" AND UNIX AND NOT APPLE AND HOMA_MKL_OPENMP_RUNTIME)
        target_link_libraries(${target_name} INTERFACE
            "-Wl,--no-as-needed"
            "${HOMA_MKL_OPENMP_RUNTIME}"
            "-Wl,--as-needed")
    endif()
endfunction()
...
if(TARGET MKL::MKL)
    ...
    if(_homa_mkl_threading_found)
        _homa_mkl_apply_threading_runtime(Homa::MKL)
...
if(_homa_mkl_found AND _homa_mkl_threading_found)
    _homa_mkl_apply_threading_runtime(Homa::MKL)
    if(HOMA_MKL_RUNTIME_DIRS)
        set_property(TARGET Homa::MKL PROPERTY homa_mkl_runtime_dirs "${HOMA_MKL_RUNTIME_DIRS}")
    endif()
